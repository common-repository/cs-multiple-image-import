 ...
 */
class CSMIISpreadsheetReader implements SeekableIterator, Countable
{
    ...
    public function __construct($Filepath, $OriginalFilename = false, $MimeType = false)
    {
        ...
        if ($this -> Type == self::TYPE_XLS)
        {
            ...
            $this -> Handle = new CSMIISpreadsheetReaderXLS($Filepath);
        ...
        switch ($this -> Type)
        {
            case self::TYPE_XLSX:
                ...
                $this -> Handle = new CSMIISpreadsheetReaderXLSX($Filepath);
                break;
            case self::TYPE_CSV:
                self::Load(self::TYPE_CSV);
                $this -> Handle = new CSMIISpreadsheetReader_CSV($Filepath, $this -> Options);
                break;
            case self::TYPE_XLS:
            ...
            case self::TYPE_ODS:
                ...
                $this -> Handle = new CSMIISpreadsheetReader_ODS($Filepath, $this -> Options);
                break;
        }
    }
...
//            require(dirname(__FILE__).DIRECTORY_SEPARATOR.'SpreadsheetReader_'.$Type.'.php');
            require(dirname(__FILE__).DIRECTORY_SEPARATOR.'CSMIISpreadsheetReader'.$Type.'.php');
        }
    }
